...
void i2c_write_byte(lcd_i2c_handle_t * lcd, uint8_t data) {
  ...
  i2c_master_cmd_begin(lcd->num, i2c_cmd, 100 / portTICK_PERIOD_MS);
...
void lcd_i2c_write(lcd_i2c_handle_t * lcd, char rs_flag, char data_byte) {
  ...
  buffer_byte = ((1 << RS) * rs_flag) | ((1 << BL) * lcd->backlight); 
  buffer_byte |= (data_byte & 0xF0); 
  buffer_byte |= (1 << EN); 
